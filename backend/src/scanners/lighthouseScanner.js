...
export const analyzeLighthouseReport = async (url, browser) => {
  ...
  try {
    // Validate browser is still connected before proceeding
    if (!browser || !browser.wsEndpoint) {
      throw new Error('Browser instance is invalid or disconnected');
    }
    ...
    try {
      ...
      throw new Error(`Failed to get browser port: ${e.message}`);
    }
